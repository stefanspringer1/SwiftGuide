...
This is an overview of “dangerous” operations or situations which could cause a Swift program to crash, in comparison to Java and C#. (Note that even if your progranm does not crash, your program still might do unexpected things e.g. because of an unnoticed overflow of a number value.)

- None of these systems can handle problems within a program caused by “memory hunger”, overflow aborts because of a high recursion level or problems in the virtual machine or corresponding problems caused by compiler errors.
