...
- a credential manager must be activated for Git if packages from private repositories have to be pulled from the Swift Package Manager; _this should not be explicitly necessary within Xcode_
...
- static linking via addition to the build command `--static-swift-stdlib`, compare the help output (`swift build --help`); the _compiler_ setting `-Xswiftc -static-executable` (link everything) or `-Xswiftc -static-stdlib` (Swift standard libraries only) that you could add to the build command should be seen as unstable options (none of the options passed via `-Xswiftc` should be considered stable)
...
- note that for the [Swift REPL](https://developer.apple.com/swift/blog/?id=18) ("[Read–eval–print loop](https://en.wikipedia.org/wiki/Read–eval–print_loop)”) **Python** must be available
...
- If your program ends unexpextectly, execute `echo %ERRORLEVEL%` right after the above execution and use the value as argument to the [The Microsoft Error Lookup Tool](https://learn.microsoft.com/en-us/windows/win32/debug/system-error-code-lookup-tool) to see what the error is.
...
- for **Swift Package Manager** ("SPM") see the [Introduction](https://www.swift.org/getting-started/#using-the-package-manager) and [Documentation](https://github.com/apple/swift-package-manager/tree/main/Documentation) or [this introductory blog post](https://www.timc.dev/posts/understanding-swift-packages/) and the explanations of the versioning concept in the section "Versioning of dependent packages" below, there in particular the section "Tested version combinations" on the question of whether the `Package.resolved` file should be versioned; get a visual dependencies graph for a package by `swift package show-dependencies --format dot | dot -Tsvg -o graph.svg` (the `dot` commands necessitates the installation of graphviz); note that besides getting packages via Git, the SPM will allow to [get packages via registry services](https://github.com/apple/swift-evolution/blob/main/proposals/0292-package-registry-service.md) in the future
- you might **search for packages** in the [Swift Package Index](https://swiftpackageindex.com)
