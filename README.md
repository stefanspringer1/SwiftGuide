...
Answer: The special thing about Swift under Windows is that Swift is implemented as a language with equal rights to C++ etc. so to speak, with access to all Windows libraries. The Swift standard library is implemented using direct calls into the Windows APIs, allowing a more "bare metal" environment compared to other environments. This system integration does not exist with Rust and Go.
