...
**files copied into the Visual Studio installation** (the environment variables `UniversalCRTSdkDir`, `VCToolsInstallDir`, and `UCRTVersion` are set within the x64 Native Tools Command Prompt of Visual Studio, `SDKROOT` should point to `%SystemDrive%\Library\Developer\Platforms\Windows.platform\Developer\SDKs\Windows.sdk\usr\share` for the standard installation):
